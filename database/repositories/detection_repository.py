"""
Detection repository - CRUD operations for extraction_detections_draft
"""
...
def get_detection(detection_id):
    """Get detection by ID"""
    result = supabase_request('GET', 'extraction_detections_draft', filters={'id': f'eq.{detection_id}'})
    return result[0] if result else None


def create_detection(data):
    """Create new detection"""
    return supabase_request('POST', 'extraction_detections_draft', data)


def update_detection(detection_id, updates):
    """Update detection by ID"""
    return supabase_request('PATCH', 'extraction_detections_draft', updates, {'id': f'eq.{detection_id}'})


def delete_detection(detection_id):
    """Delete detection by ID"""
    return supabase_request('DELETE', 'extraction_detections_draft', filters={'id': f'eq.{detection_id}'})
...
def get_detections_by_page(page_id, status=None):
    ...
    return supabase_request('GET', 'extraction_detections_draft', filters=filters) or []
...
def delete_detections_by_page(page_id):
    ...
    return supabase_request('DELETE', 'extraction_detections_draft', filters={'page_id': f'eq.{page_id}'})
...
def batch_create_detections(detections_list):
    ...
    return supabase_request('POST', 'extraction_detections_draft', detections_list) or []
